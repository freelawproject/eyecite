...
from eyecite.reporter_tokenizer import EDITIONS_LOOKUP, ReporterToken, Token
...
class FindTest(TestCase):
    def run_test_pairs(self, test_pairs, message):
        for q, expected_cites, *kwargs in test_pairs:
            ...
            match = [type(i) for i in cites_found] == [
            ...
            self.assertTrue(
                ...
                % (
                    ...
                    ",\n".join(
                        [str(cite.__dict__) for cite in expected_cites]
                    ),
                ),
            )
            print("✓")
    ...
    def test_custom_tokenizer(self):
        def tokenizer(text):
            """Simple tokenizer that recognizes just the reporter U.S.,
            but allows periods to come through OCR as commas."""
            us_edition = EDITIONS_LOOKUP["U.S."][0]
            return [
                ReporterToken(s, [us_edition], [])
                if re.match(r"U[.,]S[.,]$", s)
                else Token(s)
                for s in text.split()
            ]
        ...
        # fmt: on
        self.run_test_pairs(test_pairs, "Testing custom tokenizer for %s...")
